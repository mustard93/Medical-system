define("project-dt/controllers-otherOutOrder", ['project-dt/init'], function(){
  /**
   * [otherOutOrderEdit 销售单控制器模块]
   * @method otherOutOrderEdit
   ...
   */
  function otherOutOrderEdit($scope, modal, alertWarn, requestData, alertOk, alertError, utils, dialogConfirm, AmountCalculationService){
    ...
    $scope.submitFormAfter = function() {
      if ($scope.submitForm_type == 'exit') {
        $scope.goTo({tabHref:'#/otherOutOrder/query.html',tabName:'其他业务单-出库'});
        return;
     ...
     if ($scope.submitForm_type == 'submit') {
       _url='rest/authen/otherOutOrder/startProcessInstance';
       data= {businessKey:$scope.formData.id};
       requestData(_url, data, 'POST')
         .then(function (results) {
           var _data = results[1];
          //  alertOk(_data.message || '操作成功');
           $scope.goTo({tabHref:'#/otherOutOrder/get.html?id='+$scope.formData.id,tabName:'其他业务单-出库'});
  ...
  .controller('otherOutOrderEdit', ['$scope', 'modal', 'alertWarn', 'requestData', 'alertOk', 'alertError', 'utils', 'dialogConfirm', 'AmountCalculationService', otherOutOrderEdit]);
});
